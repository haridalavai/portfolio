  ...
  return (
    <>
      <div
        ...
        ref={cursorRef}
        className={`fixed pointer-events-none  -translate-x-1/2 -translate-y-1/2 z-50   rounded-full w-3 h-3 bg-${cursorColor}`}
      />
      <div
        style={{ top: position.y, left: position.x }}
        ref={cursorRef}
        className={`p-0 fixed pointer-events-none  -translate-x-1/2 -translate-y-1/2 z-50  rounded-full w-8 h-8 border-2 border-${cursorColor} `}
      >
        <div
          className={`w-8 h-8 ${
